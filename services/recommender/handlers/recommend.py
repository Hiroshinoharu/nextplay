...
async def recommend_for_user(user_id: int, request: Request) -> UserRecommendResponse:
    ...
    for game in candidates:
        ...
        game_keywords = {int(k) for k in (game.get("keywords") or []) if isinstance(k, (int,float,str)) and str(k).isdigit()}
        game_platforms = {int(p) for p in (game.get("platforms") or []) if isinstance(p, (int,float,str)) and str(p).isdigit()}
