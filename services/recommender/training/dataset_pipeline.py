...
def _parse_event_ts(raw: str) -> datetime:
    """
    Parse an ISO 8601 timestamp string and return a timezone-aware datetime object.
    
    If the timestamp does not include timezone information, it will be treated as UTC.
    
    Args:
        raw (str): the ISO 8601 timestamp string to parse
    Returns:
        datetime: a timezone-aware datetime object representing the parsed timestamp
    Raises:
...
def _normalize_row(row: dict[str, str]) -> dict[str, str] | None:
    """
    Normalize a single interaction row to conform to the expected format for training.

    The normalization process involves the following steps:

    1. Extract the user_id, game_id, and event timestamp from the row.
    2. Validate that the user_id and game_id are non-empty.
    3. Parse the event timestamp into a timezone-aware datetime object.
    4. Normalize the liked and rating fields based on the label and liked fields.

    Args:
        row (dict[str, str]): the interaction row to normalize

    Returns:
        dict[str, str] | None: the normalized interaction row if successful, otherwise None
    """
...
def load_interactions_from_csv(path: Path) -> list[dict[str, str]]:
    """
    Load user-game interactions from a CSV file and return a list of normalized interaction rows.

    The function reads a CSV file from the specified path, normalizes each interaction row using the
    `_normalize_row` function, and returns a list of normalized interaction rows.

    Args:
        path (Path): the path to the CSV file containing the user-game interactions

    Returns:
        list[dict[str, str]]: a list of normalized interaction rows
    """
...
def generate_seeded_interactions(
    *,
    ...
    """
    Generate a list of user-game interactions with a seeded holdout set.

    The function generates a list of user-game interactions with a seeded holdout set.
    The holdout set contains a fixed number of games that are not included in the user's history.
    The user's history is generated by randomly selecting games from the target pool and adding them to the user's history.
    The holdout set is then added to the user's history.

    Args:
        seed (int): the seed used to generate the holdout set
        users (int): the number of users to generate
        games (int): the number of games to generate
        history_per_user (int): the number of games to add to each user's history
        holdout_per_user (int): the number of games to add to each user's holdout set

    Returns:
        list[dict[str, str]]: a list of user-game interactions with a seeded holdout set
    """
    if users < 4:
    ...
    def add_row(*, user_id: str, game_id: str, liked: str) -> None:
        """
        Add a row to the dataset with the given user_id, game_id, and liked status.
        
        The function will not add the row if the user_id and game_id pair has already been seen.
        The function will also increment the timestamp by one minute after adding the row.
        
        Args:
            user_id (str): the user_id to add to the row
            game_id (str): the game_id to add to the row
            liked (str): the liked status to add to the row
        """
        nonlocal timestamp
...
def merge_interaction_rows(
    *,
    ...
    """
    Merge two lists of interaction rows into a single deduplicated list.

    The resulting list is sorted by event timestamp.

    Args:
        seeded_rows (list[dict[str, str]]): List of interaction rows generated by the seeding process.
        external_rows (list[dict[str, str]]): List of interaction rows from an external source.

    Returns:
        list[dict[str, str]]: A single list of deduplicated interaction rows sorted by event timestamp.
    """
...
def write_interactions_csv(path: Path, rows: list[dict[str, str]]) -> None:
    """
    Write a list of interaction rows to a CSV file.

    Args:
        path (Path): The path to the CSV file to write.
        rows (list[dict[str, str]]): A list of interaction rows to write to the CSV file.
...
def build_dataset_profile(
    *,
    ...
    """
    Compute various statistics about the labeled dataset.

    Args:
        train_csv (Path): Path to the CSV file containing training data.
        validation_csv (Path): Path to the CSV file containing validation data.
        test_csv (Path): Path to the CSV file containing test data.

    Returns:
        dict[str, Any]: A dictionary containing various statistics about the labeled dataset.

    Statistics included:
        - total_labeled_rows: total number of labeled rows.
        - split_rows: a dictionary containing the number of rows in each split.
        - unique_users: number of unique users.
        - unique_games: number of unique games.
        - users_with_positive: number of users with at least one positive labeled row.
        - positive_rows: number of positive labeled rows.
        - negative_rows: number of negative labeled rows.
        - positive_ratio: ratio of positive labeled rows to total labeled rows.
...
def evaluate_dataset_quality(
    *,
    ...
    """
    Evaluate a labeled dataset against quality thresholds.

    Args:
        profile (dict[str, Any]): A dictionary containing various statistics about the labeled dataset.
        config (DatasetQualityConfig): A configuration object containing quality thresholds.

    Returns:
        list[str]: A list of quality threshold failures.

    Failures are reported in the format "<check_name>: observed=<observed_value> required=<required_value>".
    """
    failures: list[str] = []
