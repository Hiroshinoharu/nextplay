...
func TestRequireJWTRejectsMissingToken(t *testing.T) {
	...
	if got := resp.Header.Get("X-NextPlay-Auth-Error"); got != "session-invalid" {
		t.Fatalf("expected auth error header, got %q", got)
	}
...
func TestRequireJWTAcceptsSessionCookie(t *testing.T) {
	t.Setenv("JWT_SECRET", "test-secret")
	token, err := gatewayauth.CreateToken(42)
	if err != nil {
		t.Fatalf("CreateToken returned error: %v", err)
